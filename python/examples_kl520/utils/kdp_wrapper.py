...
DME_MODEL_SIZE = 40 * 1024 * 1024
DME_FWINFO_SIZE = 4096
DME_SEND_IMG_RETRY_TIME = 2
SLEEP_TIME = 0.001

ISI_IMG_SIZE = IMG_SOURCE_W * IMG_SOURCE_H * 2
# ISI initial result memory size in bytes, which is equal or larger than the real size.
...
FDR_THRESH        = 0.475
IMG_FORMAT_RGB565 = constants_kl520.IMAGE_FORMAT_SUB128 | constants_kl520.NPU_FORMAT_RGB565
img_idx           = 0
...
def get_landmark_res(dev_idx, inf_size, frames):
    ...
    lm_res = ctypes.cast(
        ctypes.byref(inf_res), ctypes.POINTER(constants_kl520.LandmarkResult)).contents
    score = lm_res.score
    blur = lm_res.blur
    print(score, blur)
...
def get_age_gender_res(dev_idx, inf_size):
    #inf_res = (ctypes.c_char * inf_size)()
    inf_res = constants_kl520.FDAgeGenderRes()
...
def get_detection_res(dev_idx, inf_size):
    ...
    fp_header_res = ctypes.cast(
        ctypes.byref(inf_res), ctypes.POINTER(constants_kl520.RawFixpointData)).contents
    output_num = fp_header_res.output_num

    outnode_params_res = ctypes.cast(
        ctypes.byref(fp_header_res.out_node_params),
        ctypes.POINTER(constants_kl520.OutputNodeParams * output_num)).contents
    ...
    for param in outnode_params_res:
        ...
        offset = ctypes.sizeof(ctypes.c_int) + output_num * ctypes.sizeof(constants_kl520.OutputNodeParams)
...
def kdp_dme_load_model(dev_idx, _model_path):
    ...
    # dme configuration
    model_id = 18      # model id when compiling in toolchain
    output_num = 1     # number of output node for the model
    image_col = 640
    image_row = 480
    image_ch = 3
    image_format = (constants_kl520.IMAGE_FORMAT_SUB128 |
                    constants_kl520.NPU_FORMAT_RGB565 |
                    constants_kl520.IMAGE_FORMAT_RAW_OUTPUT |
                    constants_kl520.IMAGE_FORMAT_CHANGE_ASPECT_RATIO)
...
def kdp_dme_load_ssd_model(dev_idx, _model_path, is_raw_output):
    ...
    if is_raw_output:
        image_format = image_format | constants_kl520.IMAGE_FORMAT_RAW_OUTPUT
...
def kdp_dme_load_yolo_model(dev_idx, _model_path, is_raw_output):
    """Load dme model."""
    model_id = 0
    p_buf = (ctypes.c_char * DME_MODEL_SIZE)()
    ret_size = 0
...
def kdp_dme_inference(dev_idx, app_id, capture, buf_len, frames):
    ...
    if (app_id == constants_kl520.APP_AGE_GENDER):
        det_res = get_age_gender_res(dev_idx, inf_size)
    elif (app_id == constants_kl520.APP_FD_LM):
        det_res = get_object_detection_res(dev_idx, inf_size, frames)
    elif (app_id == constants_kl520.APP_TINY_YOLO3):
...
def capture_cam_frame(cap):
    ...
    if sys.platform == "darwin":
        frame1 = cv2.resize(frame1, (IMG_SOURCE_W, IMG_SOURCE_H), interpolation=cv2.INTER_CUBIC)
...
def register_user(dev_idx, frame, user_id):
    ...
    if (ret):
        if (ret == constants_kl520.MSG_APP_UID_EXIST):
            print("> user exist <")
        return ret
...
def verify_user_id(dev_idx, frame):
    ...
    fd_lm_res = ctypes.cast(ctypes.byref(res), ctypes.POINTER(constants_kl520.FDLMRes)).contents
...
def isi_get_result(dev_idx, img_id, rsp_code, r_size, r_data, app_id):
    ...
    if r_size >= 4:
        if (app_id == 0): # raw output
            # Prepare for postprocessing
            inf_res = ctypes.cast(
                ctypes.byref(r_data), ctypes.POINTER(ctypes.c_char * r_size)).contents
            listdata = [ord(byte) for byte in inf_res]
            npdata = np.asarray(listdata)
            ...
            return 0, npraw_data_array, r_size
        elif app_id == constants_kl520.APP_AGE_GENDER: # age_gender
            gender = ["Female", "Male"]
            result = ctypes.cast(
                ctypes.byref(r_data), ctypes.POINTER(constants_kl520.FDAgeGenderS)).contents
            box_count = result.count
            print("Img [{}]: {} people\n".format(img_id, box_count))
            box = ctypes.cast(
                ctypes.byref(result.boxes),
                ctypes.POINTER(constants_kl520.FDAgeGenderRes * box_count)).contents

            for idx in range(box_count):
                print("[{}]: {}, {}\n".format(idx, gender[box[idx].ag_res.ismale], box[idx].ag_res.age))
        else: # od, yolo
            od_header_res = ctypes.cast(
                ctypes.byref(r_data), ctypes.POINTER(constants_kl520.ObjectDetectionRes)).contents
            box_count = od_header_res.box_count
            print("image {} -> {} object(s)\n".format(img_id, box_count))
...
def start_isi(device_index, app_id, width, height):
    ...
    if (app_id == constants_kl520.APP_OD):
        image_format = 0x80000060 | constants_kl520.IMAGE_FORMAT_CHANGE_ASPECT_RATIO  #RGB565, no parallel mode
    else:
        image_format = 0x80000060                                               #RGB565, no parallel mode
    size = ISI_DETECTION_SIZE
...
def start_isi_parallel(device_index, app_id, width, height):
    ...
    print("starting ISI mode...\n")
    if (app_id == constants_kl520.APP_OD):
        image_format = 0x88000060 | constants_kl520.IMAGE_FORMAT_CHANGE_ASPECT_RATIO  #RGB565, parallel mode
    else:
...
def start_isi_parallel_ext(device_index, app_id, width, height, is_raw_output=False):
    ...
    Arguments:
        ...
        height: Height of the input image.
        image_format: Format of input image.
        is_raw_output: Get raw output data, False by default.
    """
    print("starting ISI mode...\n")
    # isi configuration
    c_int_p = ctypes.POINTER(ctypes.c_int)
    ...
    isi_cfg = constants_kl520.KDPISIConfig(app_id, size, width, height, image_format, crop_box, pad_value, extra_param)
...
def pipeline_inference(device_index, app_id, loops, input_size, capture,
                  img_id_tx, img_left, buffer_depth, frames, post_handler, is_raw_output=False):
    """Send the rest of images and get the results.
    ...
    while loops:
        if (is_raw_output):
            ...
            _, npraw_data_array, result_size = isi_get_result(
    ...
    while buffer_depth:
        if (is_raw_output):
            app_id = 0 # for raw output
            ret, npraw_data_array, result_size = isi_get_result(
                device_index, img_id_rx, 0, 0, inf_res, app_id)
            post_handler(device_index, npraw_data_array, frames)
        else:
...
def dme_pipeline_inference(device_index, app_id, loops, input_size, capture,
    ...
    while loops:
        ...
        while 1:
            ...
            # print(status, inf_size)
            if status == 1:
                if (app_id == constants_kl520.APP_TINY_YOLO3):
...
def kdp_exit_isi(dev_idx):
    api.kdp_end_isi(dev_idx)
...
def kdp_get_crc(device_index, from_ddr):
    ...
    crcinfo = ctypes.cast(
        ctypes.byref(r_data), ctypes.POINTER(ctypes.c_uint * 256)).contents
    crc = []
    crc.append(crcinfo[0])
    # print(crc[0])
...
def kdp_get_nef_model_metadata(model_path):
    ...
    r_data = (ctypes.c_char * ctypes.sizeof(constants_kl520.NefModelMetaData))()
    # Get the metadata for nef model file
    ret = api.kdp_get_nef_model_metadata(p_buf, n_len, r_data)

    metadata = ctypes.cast(
        ctypes.byref(r_data), ctypes.POINTER(constants_kl520.NefModelMetaData)).contents
